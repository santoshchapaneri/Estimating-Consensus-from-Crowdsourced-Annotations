...
function [Yhat,f_WhoIsAdversary,VarAnn,VarAnnLB] = AnnotatorsConsensusCI_Adv(Y, alpha, maxIter)
...
for iter=1:maxIter
    ...
    for m=1:M
        ...
        % Find adversariness of mth annotator
        adv(m) = 0;
        for k = 1:numel(idx)
            adv(m) = adv(m) + sum(Z(k,:).*Yhat(idx(k),:));
        end
        adv(m) = sign(adv(m));
...
%             sumnorm = sumnorm + (norm(Z(k,:) - Yhat(idx(k),:))/stderror(idx(k),1));
            sumnorm = sumnorm + norm(Z(k,:) - adv(m)*Yhat(idx(k),:));
        end
